...
function MemoriesView({ memories, people, canEdit, createMemoryAction, updateMemoryAction, deleteMemoryAction }) {
  ...
  return (
    <div style={{ display: "flex", height: "100%", gap: 20, padding: "20px 24px" }}>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {canEdit && (
            <button
              ...
              style={{
                ...
                background: TOKENS.ink, color: TOKENS.parchment, border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600
              }}
            >
              <Plus size={16} /> Yangi xotira
            </button>
          )}
        </div>
        ...
        {showForm && canEdit && (
          <form action={formAction} style={{ background: TOKENS.card, padding: 16, borderRadius: 12, gap: 12, display: "flex", flexDirection: "column" }}>
            <input type="text" name="title" placeholder="Xotira nomi" required style={{ padding: "8px 12px", borderRadius: 6, border: `1px solid ${TOKENS.parchmentDeep}`, fontSize: 13 }} />
            <textarea name="description" placeholder="Qisqacha tavsif" style={{ padding: "8px 12px", borderRadius: 6, border: `1px solid ${TOKENS.parchmentDeep}`, fontSize: 13, minHeight: 80, fontFamily: "inherit" }} />
            ...
            <select name="personId" style={{ padding: "8px 12px", borderRadius: 6, border: `1px solid ${TOKENS.parchmentDeep}`, fontSize: 13 }}>
              <option value="">Odam tanlang (ixtiyoriy)</option>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button type="button" onClick={() => setShowForm(false)} style={{ flex: 1, padding: "10px 16px", borderRadius: 6, background: TOKENS.parchmentDeep, color: TOKENS.ink, border: "none", cursor: "pointer" }}>Bekor qilish</button>
            </div>
            {formState?.error && <div style={{ fontSize: 12, color: TOKENS.danger }}>{formState.error}</div>}
          </form>
        )}

        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16, flex: 1, overflowY: "auto" }}>
          {memoriesByDate.length === 0 ? (
            <div style={{ gridColumn: "1/-1", textAlign: "center", color: TOKENS.ink60, padding: "40px 20px" }}>
              <div style={{ fontSize: 14, marginBottom: 8 }}>Xotiralar hali qo'shilmagan</div>
              <div style={{ fontSize: 12, color: TOKENS.ink40 }}>Oilanizning muhim voqealarini va xotirasini saqlab qoling</div>
            </div>
          ) : (
            memoriesByDate.map(memory => (
              <div
                key={memory.id}
                onClick={() => setSelectedMemory(memory)}
                style={{
                  background: TOKENS.card, borderRadius: 12, overflow: "hidden", cursor: "pointer",
                  transition: "transform 0.2s, box-shadow 0.2s",
                  boxShadow: `0 2px 8px rgba(30,38,33,0.08)`,
                }}
                onMouseEnter={(e) => { e.currentTarget.style.transform = "translateY(-4px)"; e.currentTarget.style.boxShadow = `0 6px 16px rgba(30,38,33,0.12)`; }}
                onMouseLeave={(e) => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = `0 2px 8px rgba(30,38,33,0.08)`; }}
              >
                {memory.photo_url && <img src={memory.photo_url} alt="" style={{ width: "100%", height: 160, objectFit: "cover" }} />}
                <div style={{ padding: 12 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: TOKENS.ink, marginBottom: 4 }}>{memory.title}</div>
                  {memory.memory_date && <div style={{ fontSize: 11, color: TOKENS.teal }}>📅 {new Date(memory.memory_date).toLocaleDateString('uz-UZ')}</div>}
                  {memory.location && <div style={{ fontSize: 11, color: TOKENS.ink60 }}>📍 {memory.location}</div>}
                </div>
              </div>
            ))
          )}
      ...
      {selectedMemory && (
        <div style={{ width: 320, background: TOKENS.card, borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 16, maxHeight: "100%", overflowY: "auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <h2 style={{ fontSize: 16, fontWeight: 600, color: TOKENS.ink, margin: 0 }}>{selectedMemory.title}</h2>
            <button onClick={() => setSelectedMemory(null)} style={{ background: "none", border: "none", cursor: "pointer", color: TOKENS.ink40 }}><X size={18} /></button>
          </div>
          {selectedMemory.photo_url && <img src={selectedMemory.photo_url} alt="" style={{ width: "100%", height: 200, objectFit: "cover", borderRadius: 8 }} />}
          {selectedMemory.memory_date && <div style={{ fontSize: 13, color: TOKENS.teal }}>📅 {new Date(selectedMemory.memory_date).toLocaleDateString('uz-UZ')}</div>}
          {selectedMemory.location && <div style={{ fontSize: 13, color: TOKENS.ink60 }}>📍 {selectedMemory.location}</div>}
          {selectedMemory.description && <div style={{ fontSize: 13, color: TOKENS.ink60, lineHeight: 1.6 }}>{selectedMemory.description}</div>}
          {canEdit && (
            <button
              onClick={() => {
                const fd = new FormData();
                fd.append("memoryId", selectedMemory.id);
                deleteMemoryAction(fd);
                setSelectedMemory(null);
              }}
              style={{ padding: "10px 16px", borderRadius: 6, background: TOKENS.danger, color: TOKENS.parchment, border: "none", cursor: "pointer", fontWeight: 600, fontSize: 12 }}
            >
              <Trash2 size={14} style={{ marginRight: 6 }} /> O'chirish
            </button>
          )}
        </div>
      )}
